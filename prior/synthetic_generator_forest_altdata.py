...
def synthetic_dataset_function_forest_altdata(
        get_data = np.random.normal,
        min_features = 3,
        max_features = 100,
        n_samples = 10000,
        max_classes = 10,
    ...
    x = get_data(size=(base_size, n_features))
    ...
    x2 = get_data(size=(n_samples, n_features))
...
def synthetic_dataset_generator_forest_altdata(**kwargs):

    while True:
        x, y = synthetic_dataset_function_forest_altdata(**kwargs)
        yield x, y
